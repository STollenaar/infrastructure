...
resource "hcp_vault_secrets_secret" "route53_user_access_key" {
  app_name     = hcp_vault_secrets_app.aws.app_name
  secret_name  = "route53_access_key"
  secret_value = aws_iam_access_key.route53_user_key.id
}

resource "hcp_vault_secrets_secret" "route53_user_secret_access_key" {
  app_name     = hcp_vault_secrets_app.aws.app_name
  secret_name  = "route53_secret_access_key"
  secret_value = aws_iam_access_key.route53_user_key.secret
}
